...
# def rank_key_moments_with_groq(segments):
#     """
#     Use GROQ LLM to rank segments by importance.
#     Returns list of (index, score).
#     """

#     if not GROQ_API_KEY:
#         # Fall back to original order if key is missing
...
#     prompt = f"""
# You are analyzing a transcript split into segments.
# Score each segment from 0–100 for how important or "key moment" it is.
...
#         ranked = json.loads(data)

#         # Return a list of segment indexes sorted by score desc
#         ranked_sorted = sorted(ranked, key=lambda x: x["score"], reverse=True)
#         return [item["index"] for item in ranked_sorted]

#     except Exception as e:
#         print("Key moment ranking failed:", e)
#         return list(range(len(segments)))  # fallback
